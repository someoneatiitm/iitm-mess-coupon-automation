...
### Where to Reply in WhatsApp

All bot commands are sent to **your own chat** (Saved Messages / your own number):
...
│                                                                 │
│  ┌─────────────────────────┐                                    │
│  │ 💬 You (Saved Messages) │ ← Reply HERE to control the bot   │
│  │    "ok", "paid", "stop" │                                    │
│  └─────────────────────────┘                                    │
│                                                                 │
│  ┌─────────────────────────┐                                    │
│  │ 👤 Rahul (Seller)       │ ← Bot chats here automatically    │
│  │    Bot handles this     │                                    │
│  └─────────────────────────┘                                    │
│                                                                 │
│  ┌─────────────────────────┐                                    │
│  │ 👥 Buy & Sell Group     │ ← Bot monitors this for sellers   │
│  │    Bot reads only       │                                    │
│  └─────────────────────────┘                                    │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
...
1. Open WhatsApp → New Chat → Search your own name/number
2. Or message yourself once, it'll appear in your chat list
3. All bot notifications come here, reply here to respond
...
```
┌─────────────────────────────────────────────────────────────────┐
│  IITM Mess Coupon Bot                          [Logout] [Restart]│
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  ┌──────────────────┐  ┌──────────────────┐                     │
│  │   📱 QR CODE     │  │   📊 STATUS      │                     │
│  │                  │  │                  │                     │
│  │  [QR appears     │  │  Lunch: NEEDED   │                     │
│  │   here on first  │  │  Pref: Firstman  │                     │
│  │   launch]        │  │                  │                     │
│  │                  │  │  Dinner: NEEDED  │                     │
│  │  Scan with       │  │  Pref: Any       │                     │
│  │  WhatsApp        │  │                  │                     │
...
│  │                                                              ││
│  │  Lunch Preference:  [Any ▼]    Dinner Preference: [Any ▼]   ││
│  └─────────────────────────────────────────────────────────────┘│
│                                                                  │
│  ┌─────────────────────────────────────────────────────────────┐│
...
│  │                                                              ││
│  │  ┌────────────────────────────────────────────────────────┐ ││
│  │  │ Rahul • Lunch • ₹70 • Firstman                         │ ││
│  │  │ State: AWAITING_COUPON                                 │ ││
│  │  │ Messages:                                              │ ││
│  │  │   You: "saw your message, still available?"            │ ││
│  │  │   Rahul: "yes"                                         │ ││
│  │  │   You: "which mess?"                                   │ ││
│  │  │   Rahul: "firstman"                                    │ ││
│  │  │   You: "UPI ID?"                                       │ ││
│  │  │   Rahul: "9876543210@paytm"                            │ ││
│  │  └────────────────────────────────────────────────────────┘ ││
│  └─────────────────────────────────────────────────────────────┘│
│                                                                  │
│  ┌─────────────────────────────────────────────────────────────┐│
...
|---------|--------------|
| **QR Code** | Scan with WhatsApp on first launch to connect your account |
| **Status** | Shows if lunch/dinner is needed, bought, or paused |
| **Controls** | Start/Stop searching, Reset sessions, Change preferences |
| **Preference Dropdowns** | Select which mess you want (SGR, SRR, Firstman, etc.) |
| **Active Conversations** | Live view of ongoing chats with sellers |
| **Pending Confirmation** | Click to approve/decline a deal (same as typing "ok"/"no") |
| **Payment Pending** | Click after you've sent UPI payment (same as typing "paid") |

#### Dashboard vs WhatsApp - Both Work!

You can control the bot from **either** place:

| Action | WhatsApp (Your Chat) | Dashboard |
|--------|---------------------|-----------|
| Confirm deal | Reply `ok` | Click "Confirm Purchase" |
| Decline deal | Reply `no` | Click "Decline" |
| Mark paid | Reply `paid` | Click "I've Paid" |
| Stop searching | Reply `stop` | Click "Stop" |
| Change preference | Reply `hi` then number | Use dropdown |
| Check status | Reply `status` | Always visible |

**Tip:** Keep the dashboard open in a browser tab - it shows real-time updates and is faster than typing on WhatsApp.
